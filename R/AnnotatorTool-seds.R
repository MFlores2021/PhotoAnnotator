...
get.cipnumber.from.txt <-function(txt){
	...
	cpn = "(CIP)?[1-4]{1}[0-9@yDZO/]{5}(\\.[0-9@yDZO]{1,4})?" 
	
	#make sure to preced cip number with CIP snap-ea665adb
...
compare.result <- function(cipz,cipt,list.cip) {
	...
	if(!is.null(cip)) {
		if (length(cip)>1) {		
			...
			cipnumber = paste(strcip,cip, sep = "")
		}
	}
		...
		tryCatch({
			...
			if (!length(CIPN) || is.null(CIPN)){
				...
				if (!length(CIPN) || is.null(CIPN)){				
					...
					#if(length(CIPN) || !(is.null(CIPN))) CIPN = paste("R_", CIPN, sep="")
				}
			}
					#CIPN = paste(CIPZ,CIPT,sep="-")	
...
menu.rename <- function(dir_initial,fbp,locality,ppl,yeardate,autor){
	...
	if (length(oldfiles)){
		...
		for(i in 1:length(oldfiles)) {
			...
			if (length(CIPN)){				
				...
				count=2
				new_name = file.path(dir_final,paste(CIPN,'_',yeardate,'_',part_plant,'.jpg',sep=''))
								
				while(new_name %in% file_exists){ 		
				
					new_name = file.path(dir_final,paste(CIPN,'_',yeardate,'_',part_plant,count,'.jpg',sep=''))
...
get.org.list= function(){
	...
	org = c('plant','flower','flower dissection','fruit','seeds','tuber','root','sprout','herbarium','habitat','foliage','chips')
	return(org) 
}
...
create.img.Dlg = function(w){
	...
	win <- gbasicdialog(title='Add images to experiment', handler = function(h,...) {
			...
			if(!is.na(svalue(gl[8,1])) && !is.na(svalue(gl[1,1]))){
					...
					result = c(svalue(gl[1,2]),svalue(gl[2,2]),svalue(gl[8,1]),svalue(gl[3,2]),"",svalue(gl[6,2]))
	...
	#gl[5,1]=glabel("Localities:", cont=gl)
	#gl[5,2]=gcombobox(get.localities(), cont=gl) 
	gl[6,1]=glabel("Year:", cont=gl)
	gl[6,2]=gedit("13", cont=gl)
